...
// MTLK: This holds the temporary allocations high water mark
static uint8_t* _max_temp = nullptr;
// MLTK: This is populated when GetUsedBytes() is called
uint32_t required_tensor_arena_bytes = 0;
...
uint8_t* SimpleMemoryAllocator::AllocatePersistentBuffer(size_t size,
                                                         size_t alignment) {
 // MLTK: Keep alignment consistent for non-embedded targets
#ifndef __arm__
  if(alignment == 8)
  {
    alignment = 4;
...
bool SimpleMemoryAllocator::IsAllTempDeallocated() {
  // FIXME: Disabling this check for now until all the kernels are updated
...
size_t SimpleMemoryAllocator::GetUsedBytes() const {
  // MLTK: Update the global variable to return the maximum number of bytes used
  //       including temporary allocations and padding
  // +128 for any additional temp allocation during inference (specifically GetInput() and GetOutput())
  required_tensor_arena_bytes = (GetBufferSize() - (AlignPointerDown(tail_, 16) - AlignPointerUp(_max_temp, 16))) + sizeof(SimpleMemoryAllocator) + 128;
