...
#define SLI_MVP_CHECK(x, ...)                \
  do {                                  \
    if(status != SL_STATUS_OK)  return __VA_ARGS__; \
    if (!(x)) {                         \
      status = SL_STATUS_INVALID_RANGE; \
      MLTK_KERNEL_UNSUPPORTED_MSG("Assertion failed: " STRINGIFY(x)); \
      return __VA_ARGS__;               \
    }                                   \
  } while (0)
...
struct DwConv2DDims {
  ...
  void mvp_incr_index_stream(sli_mvp_program_context_t *p, int dim, int stream_no, int reg_no)
  {
    if(p->last_instr >= 8) {
      ...
      MLTK_KERNEL_UNSUPPORTED_MSG("Instruction overflow");
      return;
    }
  ...
  {
    if((p->loop_level + 1) >= 8) {
      MLTK_KERNEL_UNSUPPORTED_MSG("Loop overflow");
      status = SL_STATUS_INVALID_PARAMETER;
      return;
    }
  ...
  for(int dim = 0; dim < kDimCount; ++dim) {
    if (input_dims.isActive(dim)) {
      if ((unsigned)input_dims.sizes[dim] > SLI_MVP_MAX_VECTOR_COUNT) {
        MLTK_KERNEL_UNSUPPORTED_MSG("input_shape[%d] (%d) > %d", dim, input_dims.sizes[dim], SLI_MVP_MAX_VECTOR_STRIDE);
        return SL_STATUS_INVALID_PARAMETER;
      }
    }
    if (output_dims.isActive(dim)) {
      if ((unsigned)output_dims.sizes[dim] > SLI_MVP_MAX_VECTOR_COUNT) {
        MLTK_KERNEL_UNSUPPORTED_MSG("output_shape[%d] (%d) > %d", dim, output_dims.sizes[dim], SLI_MVP_MAX_VECTOR_COUNT);
        return SL_STATUS_INVALID_PARAMETER;
      }
      if ((unsigned)output_dims.strides[dim] > SLI_MVP_MAX_VECTOR_STRIDE) {
        MLTK_KERNEL_UNSUPPORTED_MSG("output_stride[%d] (%d) > %d", dim, output_dims.strides[dim], SLI_MVP_MAX_VECTOR_STRIDE);
        return SL_STATUS_INVALID_PARAMETER;
      }
    }
  }
    ...
    for (int out_y_min = 0, out_y_max; out_y_min < output_height; out_y_min = out_y_max + 1) {
      ...
      if(((unsigned)in_dims.numActiveDims() > SLI_MVP_MAX_DIM)
        ...
        if (in_dims.isActive(kDimOutYOffset) && in_dims.isActive(kDimOutX)) {
          ...
          SLI_MVP_CHECK((unsigned)in_dims.sizes[kDimOutX] <= SLI_MVP_MAX_VECTOR_COUNT, SL_STATUS_INVALID_PARAMETER);
        }
      }
      ...
      SLI_MVP_CHECK((unsigned)out_dims.numActiveDims() <= SLI_MVP_MAX_DIM, SL_STATUS_INVALID_PARAMETER);
      ...
      if (out_dims.isActive(kDimOutYGroup) && (unsigned)out_dims.strides[kDimOutYGroup] > SLI_MVP_MAX_VECTOR_STRIDE) {
        ...
        SLI_MVP_CHECK((out_dims.strides[kDimOutYGroup] % out_y_offsets) == 0, SL_STATUS_INVALID_PARAMETER);
        ...
        while ((unsigned)out_dims.strides[kDimOutYGroup] > SLI_MVP_MAX_VECTOR_STRIDE) {
          SLI_MVP_CHECK((out_dims.strides[kDimOutYGroup] % 2) == 0, SL_STATUS_INVALID_PARAMETER);
      ...
      if ((unsigned)in_dims.numActiveDims() > SLI_MVP_MAX_DIM) {
        SLI_MVP_CHECK(in_dims.isActive(kDimInChannel), SL_STATUS_INVALID_PARAMETER);
      ...
      SLI_MVP_CHECK((unsigned)in_dims.numActiveDims() <= SLI_MVP_MAX_DIM, SL_STATUS_INVALID_PARAMETER);
      ...
      for (int batch = 0; batch < batches; ++batch) {
        ...
        for(int in_channel_group = 0; in_channel_group < in_channel_groups; ++in_channel_group) {
          ...
          SLI_MVP_CHECK((in_channel_group == 0) || (num_in_channels_per_program == 1), SL_STATUS_INVALID_PARAMETER);  // check assumption above
          ...
          if (parallelize_channels) {
            sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R0, (float16_t)SLI_MVP_ACCUMULATOR_SCALER, (float16_t)SLI_MVP_ACCUMULATOR_SCALER);
            ...
            sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R2, (float16_t)output_offset, (float16_t)output_offset);
          } else {
            sli_mvp_prog_set_reg_f16(p->p, SLI_MVP_R0, (float16_t)SLI_MVP_ACCUMULATOR_SCALER);
          ...
          if (execute) {
            ...
            MLTK_PROFILER_INCREMENT_PARALLEL_PROG_COUNT(parallelize_channels ? 1 : 0);
          } else {
            mvp_prog_cnt++;
          }
        } // in_channel_groups
