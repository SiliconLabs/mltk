  ...
  for (int batch = 0; batch < batches; ++batch) {
    for (int out_y = 0; out_y < output_height; ++out_y) {
      for (int out_x = 0; out_x < output_width; ++out_x) {
        ...
        sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R1, (float16_t)SLI_MVP_ACCUMULATOR_SCALER, (float16_t)SLI_MVP_ACCUMULATOR_SCALER);
        ...
        if (execute) {
          ...
          MLTK_PROFILER_INCREMENT_PARALLEL_PROG_COUNT(parallel == 2 ? 1 : 0);
        ...
        for (int batch = 0; batch < batches; ++batch) {
          ...
          for (int channel = 0; channel < depth; channel += parallelization) {
            ...
            if (output_stride_vec > (int)SLI_MVP_MAX_VECTOR_STRIDE) {
              ...
              //EFM_ASSERT(false);
              MLTK_KERNEL_UNSUPPORTED_MSG("Output vector stride (%d) exceeded (max=%d)", output_stride_vec, SLI_MVP_MAX_VECTOR_STRIDE)
            }
